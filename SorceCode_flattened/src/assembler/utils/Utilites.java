...
import assembler.entity.Instruction;
...
public class Utilites {
	...
	private static final String[] regname = new String[] { "t0",
	...
	public static List<Instruction> decodeInstruction(String src) throws SyntaxException, InvalidInstructionException {
		...
		while (scanner.hasNext()) {
			...
			if (m.matches()) {
				...
				Instruction inst = Instruction.createInstruction(sinst, lineNo, Integer.parseInt(saddr, 16));
				instList.add(inst);
			} else {
	...
	public static int extendInt20(int data, int bits, boolean zeroExtend) {
		boolean minus = (!zeroExtend) && ((Math.pow(2, bits) / 2) < data);
		String datastr = Integer.toBinaryString(data);
		while (datastr.length() < bits) {
			datastr = '0' + datastr;
		}
		while (datastr.length() < 20) {
			datastr = (minus ? '1' : '0') + datastr;
		}
		return (int) Long.parseLong(datastr, 2);
	}
